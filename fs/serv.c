...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	int bytes_to_read = req->req_n;
	if(!(bytes_to_read <= PGSIZE))
		bytes_to_read = PGSIZE;
	struct OpenFile *open_file;
	int retval = openfile_lookup(envid, req->req_fileid, &open_file);
	if(retval < 0){
		printf("\n serve_read() Failed! : No open file for this envid");
		return retval;
	}
	int bytes_read = file_read(open_file->o_file, ret->ret_buf, bytes_to_read, open_file->o_fd->fd_offset);
	if (bytes_read < 0){
		printf("\n server_read() Failed! : Error in reading-file_read()");
		return bytes_read;
	}
	open_file->o_fd->fd_offset = open_file->o_fd->fd_offset + bytes_read;
	return bytes_read; 
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int retval = openfile_lookup(envid, req->req_fileid, &open_file);
	if(retval < 0){
		printf("\n serve_write() failed! : No open file for this env\n");
		return retval;
	}
	int bytes_to_write = req->req_n;
	
	//if(bytes_to_write > PGSIZE)
	if(bytes_to_write > PGSIZE-(sizeof(int) + sizeof(size_t)))
		bytes_to_write = PGSIZE;

	int bytes_written = file_write(open_file->o_file, req->req_buf, bytes_to_write, open_file->o_fd->fd_offset);
	if(bytes_written < 0){
		printf("\n serve_write() failed! : file write failure.\n");
		return bytes_written;
	}
	open_file->o_fd->fd_offset = open_file->o_fd->fd_offset + bytes_written;
	return bytes_written;
