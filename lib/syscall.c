...
{
	return  syscall(SYS_recieve_packet,0, (uint32_t)packet, (int)size, 0, 0, 0);
}
...
sys_get_mac_address(void *mac_address){
	return syscall(SYS_get_mac_address, 1, (uint32_t)mac_address, 0, 0, 0, 0);
}
int
sys_env_assign_priority(int priority_number) {
	return syscall(SYS_env_assign_priority, 1, priority_number, 0, 0, 0, 0);
}
